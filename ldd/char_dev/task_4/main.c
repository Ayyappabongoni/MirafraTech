/*Task-4:
Write "Mirafra Technologies Hyderabad" string(from user space )into the driver and 
check that string  in driver whether received or not.
*/
...
static ssize_t my_read(struct file *flip, char __user *buffer, size_t length, loff_t *off)
{
	...
	for(i=0;i<length;i++)
	{
	buffer[i]=s[i];//sending data to usr space
	}
	buffer[i]=0;
	return i;
}
...
static int __init start_fun(void)
{
	...
	alloc_chrdev_region(&dev_no,0,1,"STRING");
