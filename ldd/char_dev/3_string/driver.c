...
static ssize_t my_write(struct file *flip, const char __user *buffer, size_t length, loff_t *off)
{
        ...
        printk("cursir position  is %lld\n",*off);
        copy_from_user(s+*off,buffer,length);//user to kernel
	*off += length;//cpy length to offset cursor moves to that length and ready for nxt string
	printk("copyed data to g_var is %s\n",s);
        return 0;
}
...
static int __init start_fun(void)
{
        ...
        alloc_chrdev_region(&dev_no,0,1,"3_strings");
        ...
        //device create
        device_create(k_class,NULL,dev_no,NULL,"3_strings");
        return 0;
}
